...
| Test | What it does |
|------|----------------|
| **xml-test-c** | Single C executable that runs all cmocka unit tests in one go (parsing, file-based, attributes, NULL/lifecycle, realloc-failure, processing instructions, CDATA, name production, unique attributes, ampersand rejection, namespaces, entities, DOCTYPE, encoding). It is built from a runner (`test/test_main.c`), per-feature test modules (`test/unit-c.c`, `test/unit-c-null.c`, `test/unit-c-pi.c`, `test/unit-c-cdata.c`, `test/unit-c-name-production.c`, `test/unit-c-unique-attributes.c`, `test/unit-c-ampersand-reject.c`, `test/unit-c-namespace.c`, `test/unit-c-entities.c`, `test/unit-c-doctype.c`, `test/unit-c-encoding.c`), and `test/wrap_realloc.c` (for mocking realloc in one test); produces **one comprehensive report** for the whole suite. |
